...
//        let topScreen    = screens.max(by: { $0.frame.minY < $1.frame.minY })!
//
//        // ウィンドウの属するスクリーンを取得（タイトルバー部分で判定）
//        // nil の場合は bottomScreen にフォールバック
//        let currentScreen = window.screen ?? bottomScreen
//
//        // 対象スクリーンの visibleFrame を使いたい場合は frame → visibleFrame に読み替え
//        let targetVisible = currentScreen.visibleFrame
//        let scale = min(targetVisible.width  / imgSize.width,
//                        targetVisible.height / imgSize.height)
//
...
//        let newX = min(max(currentOrigin.x, minX), maxX)
//
//        // Y の分岐：上下スクリーンの物理下端（frame.minY）を使用
//        let newY: CGFloat
//        if currentScreen == topScreen {
//            // ウィンドウが上段スクリーン上
//            newY = topScreen.frame.minY
//        } else {
//            // 下段スクリーン上 or フォールバック
//            newY = bottomScreen.frame.minY
//        }
//
//        // フレーム適用
//        window.setFrame(
...
//        // 上段スクリーンを取得
//        let screens = NSScreen.screens
//        guard let topScreen = screens.max(by: { $0.frame.minY < $1.frame.minY }) else { return }
//        let newY = topScreen.frame.minY
//        
//        
////        print("currentOrigin.y \(currentOrigin.y)")
////        print("newY \(newY)")
////        // フレーム適用
//        window.setFrame(
//            NSRect(x: newX, y: newY, width: newWidth, height: newHeight),
